...
class PendingDocChunkerV9:
    """待审文档分块器 v9（按真实多级标题顺序切分 + 相邻单元打包 + 表格保护）"""
    ...
    def _marker_levels(self) -> List[Dict[str, Any]]:
        ...
        return [
            ...
            },
            {
                ...
                # 真实待审文档第二级以 1. / 2. 为主；也兼容 1、/1．，避免把 1.8 m 当标题。
    ...
    def _split_content_to_units(self, chunk: Dict, content: str,
        ...
        for marker, body in parts:
            ...
            if len(full_text) > limit and idx < len(levels):
                ...
                # 含表格时仍允许按标题边界继续切；只禁止后续按表格行或句子把表格打碎。
                # 从当前层级继续扫描，可以处理中间层级缺失或同层编号嵌套的 OCR 结果。
...
def generate_visualization(all_results: Dict[str, List[Dict]], output_dir: Path):
    ...
    html_parts = [
        ...
        "<html><head>",
        "<meta charset='utf-8'>",
        "<title>待审文档 Chunk 分析报告 v9</title>",
        "<style>",
    ...
    html_path = output_dir / f"pending_doc_chunks_v9_{timestamp}.html"
    with open(html_path, 'w', encoding='utf-8') as f:
        f.write('\n'.join(html_parts))
    print(f"\n可视化报告已保存: {html_path}")

    # 保存JSON
    json_path = output_dir / f"pending_doc_chunks_v9_{timestamp}.json"
...
def main():
    ...
    for json_file in json_files:
        try:
            chunker = PendingDocChunkerV9(str(json_file))
