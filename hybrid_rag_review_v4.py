...
class HybridRAGReviewerV4:
    """混合检索RAG审核器 v4（三智能体：分类+初审+验证）"""
...
"""
    ...
    def save_results(self, results: Dict, output_path: str = None):
        ...
        if output_path is None:
            ...
            output_path = OUTPUT_DIR / f"review_result_v4_{timestamp}.json"
        # 保存时不含kb全文（减小体积）
        slim = {}
    ...
    def generate_report(self, results: Dict, output_path: str = None):
        ...
        if output_path is None:
            timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
            output_path = OUTPUT_DIR / f"review_report_v4_{timestamp}.html"
...
""" % json.dumps(kb_full_texts, ensure_ascii=False)
        ...
        parts = [
            ...
            "<title>煤矿作业规程合规审核报告 v4</title>",
            f"<style>{style}</style>",
            "</head><body>",
            # 头部
            "<div class='page-header'>",
            "<h1>煤矿作业规程合规审核报告 v4</h1>",
...
def main():
    print("=" * 70)
    print("煤矿作业规程合规审核系统 v4 - 三智能体混合检索RAG")
