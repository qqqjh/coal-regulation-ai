...
- 待审文档：pending_doc_chunking_v2.py 切分的待审文档
...
class HybridRAGReviewer:
    ...
    def review_chunk_with_llm(self, pending_chunk: Dict,
        ...
        prompt = f"""你是一位煤矿安全法规审核专家。请根据提供的法规知识库内容，审核以下待审文档内容是否符合相关法规要求。
...
请从以下几个方面进行审核：
1. 合规性：待审内容是否符合法规要求？如有不符，请指出具体条款。
2. 完整性：待审内容是否遗漏了法规要求的重要内容？
3. 准确性：待审内容中的数值、参数是否与法规一致？
4. 建议：如有问题，请给出具体的修改建议。

请以JSON格式输出审核结果：
{{
    "compliance_status": "合规/部分合规/不合规/无法判断",
    "issues": [
        {{
            "type": "问题类型（合规性/完整性/准确性）",
            "description": "问题描述",
            "reference": "相关法规条款",
            "suggestion": "修改建议"
        }}
...
"""
        ...
        try:
            response = self.llm_client.chat.completions.create(
                model=QWEN_MODEL,
                messages=[
                    {"role": "system", "content": "你是煤矿安全法规审核专家，请严格按照JSON格式输出审核结果。"},
    ...
    def review_pending_document(self, pending_json_path: str = None) -> Dict:
        """审核整个待审文档"""
        if pending_json_path is None:
            # 找最新的 v2 pending chunks 文件
            v2_files = sorted(PENDING_CHUNKS_DIR.glob("pending_doc_chunks_v2_*.json"), reverse=True)
            if not v2_files:
                raise FileNotFoundError("未找到待审文档chunks文件")
            pending_json_path = v2_files[0]
    ...
    def generate_report(self, results: Dict, output_path: str = None):
        ...
        # 统计
        total_chunks = 0
        status_counts = {'合规': 0, '部分合规': 0, '不合规': 0, '无法判断': 0}
        ...
        html_parts.append(f"部分合规: <span class='status-badge status-partial'>{status_counts['部分合规']}</span> ")
