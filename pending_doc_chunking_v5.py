...
5. v5新增：处理数字完全丢失变成孤立·的情况（如"0.9m"→"·"）
   - 识别模式类型而非仅修复个例
   - "控制在·" → 删除孤立·或标记为[数据缺失]
   - 中文后直接跟·再跟单位 → 删除·
"""
...
def generate_visualization(all_results: Dict[str, List[Dict]], output_dir: Path):
    ...
    html_parts = [
        ...
        "<html><head>",
        "<meta charset='utf-8'>",
        "<title>待审文档 Chunk 分析报告 v5</title>",
        "<style>",
    ...
    html_path = output_dir / f"pending_doc_chunks_v5_{timestamp}.html"
    with open(html_path, 'w', encoding='utf-8') as f:
        f.write('\n'.join(html_parts))
    print(f"\n可视化报告已保存: {html_path}")

    # 保存JSON
    json_path = output_dir / f"pending_doc_chunks_v5_{timestamp}.json"
...
def main():
    ...
    for json_file in json_files:
        try:
            chunker = PendingDocChunkerV5(str(json_file))
