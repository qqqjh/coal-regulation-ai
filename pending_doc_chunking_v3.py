...
class PendingDocChunkerV3:
    ...
    def clean_text(self, text: str) -> str:
        ...
        # 0. 预处理：修复被误识别为 \cdot 的范围符号
        # 例如 "1 0 \mathsf { m } \cdot 3 0 \mathsf { m }" 应该是 "10m-30m"
        # 先把 \mathsf{m} \cdot 这种模式中的 \cdot 替换为 -
        text = re.sub(r'(\\mathsf\s*\{\s*m\s*\})\s*\\cdot\s*(\d)', r'\1-\2', text)
        # 处理更通用的情况：数字+单位+\cdot+数字 → 数字+单位-数字
        text = re.sub(r'(\d)\s*(\\mathsf\s*\{\s*m\s*\})\s*\\cdot\s*(\d)', r'\1\2-\3', text)
        # 处理 \cdot 后直接跟数字的情况（约等于符号）: "\cdot 200" → "~200"
        text = re.sub(r'\\cdot\s*(\d)', r'~\1', text)
        # 处理 数字 \cdot 数字 的情况（范围符号）: "2 \cdot 5.4" → "2~5.4"
        ...
        # 4. 修复数值范围格式: "10 - 11" → "10-11", "86 106" → "86~106"
        # 处理带空格的范围符号 "10 - 11" → "10-11"
        text = re.sub(r'(\d+)\s*-\s*(\d+)', r'\1-\2', text)
        # 处理带空格的波浪号范围 "86 ~ 106" → "86~106"
        text = re.sub(r'(\d+)\s*~\s*(\d+)', r'\1~\2', text)
        # 处理可能被误分割的数值范围 "86 106" (两个相近数字中间只有空格，可能是范围)
        # 这个比较危险，先不处理，因为可能误伤正常数字
        ...
        # 7. 修复单位格式（字母间空格）
        # 先删除字母间的空格 (m m -> mm, k g -> kg)
        text = re.sub(r'([a-zA-Z])\s+([a-zA-Z])', r'\1\2', text)
        # 确保数字和单位之间有空格 (50mm -> 50 mm)
        text = re.sub(r'(\d)([a-zA-Z])', r'\1 \2', text)

        # 8. 修复单位后的范围符号格式 (10 m -30 m -> 10 m-30 m)
...
def generate_visualization(all_results: Dict[str, List[Dict]], output_dir: Path):
    ...
    html_parts = [
        ...
        "<html><head>",
        "<meta charset='utf-8'>",
        "<title>待审文档 Chunk 分析报告 v3</title>",
        "<style>",
    ...
    html_path = output_dir / f"pending_doc_chunks_v3_{timestamp}.html"
    with open(html_path, 'w', encoding='utf-8') as f:
        f.write('\n'.join(html_parts))
    print(f"\n可视化报告已保存: {html_path}")

    # 保存JSON
    json_path = output_dir / f"pending_doc_chunks_v3_{timestamp}.json"
...
def main():
    ...
    for json_file in json_files:
        try:
            chunker = PendingDocChunkerV3(str(json_file))
