...
目标：
1. 保留 v7 对待审文档段落结构、表格内容、父级标题上下文的处理。
2. 避免 v7 "一个二级条目一个 chunk" 导致的过细和颗粒度不齐。
3. 避免旧版只按长度合并、超长后按句号打碎结构的问题。

核心策略：
- 先按章/节/条形成初始 chunk。
- 再识别待审文档常见编号层级：一、二、三；1、2、3；（1）（2）（3）。
- 按结构单元切出候选单元，再把相邻同父级单元打包到约 1000 字。
- 所有结果再次检查超长，优先继续按下一级编号拆，最后才按段落/句子兜底。
- 表格作为完整内容保留，不按表格行粗暴切碎。
"""
import json
import re
...
# 普通 chunk 的硬上限。超过后会继续尝试按下一级结构或段落/句子兜底拆。
MAX_CHUNK_SIZE = 2000
# 表格 chunk 允许略长，避免把一张表格按行切碎。
TABLE_MAX_CHUNK_SIZE = 2400
# 小块下限。低于此值的结构单元会尽量并入相邻同父级单元。
MIN_CHUNK_SIZE = 300
# 纯标题或残余碎片阈值。
TINY_CHUNK_SIZE = 120
# 每个结构层级至少要有两个有效条目才触发该层级拆分。
MIN_SPLIT_PARTS = 2
# 各层级条目正文下限，避免把短标题、OCR 残片当成有效切分单元。
...
class PendingDocChunkerV8:
    """待审文档分块器 v8（结构单元切分 + 相邻单元打包 + 表格保护）"""
    ...
    def _marker_levels(self) -> List[Dict[str, Any]]:
        """待审文档常见条目层级，从粗到细。"""
        return [
            {
                'name': 'first_level_cn',
            ...
            {
                'name': 'second_level_arabic',
                # 支持 1、 / 1. / 1．，并额外支持 1.2.3 这类标题；避免把 1.8 m 当标题。
                'pattern': r'^[ \t]*(\d+(?:\.\d+){2,4}\s+|\d+[、]|(?:\d+[.．](?=\s|[\u4e00-\u9fff])))[ \t]*',
                'min_body': SECOND_LEVEL_MIN_BODY_CHARS,
            },
            {
                'name': 'third_level_parenthesized',
                'pattern': r'^[ \t]*([（(]\d+[）)])[ \t]*',
                'min_body': THIRD_LEVEL_MIN_BODY_CHARS,
            },
            {
                'name': 'fourth_level_alpha',
                'pattern': r'^[ \t]*([a-zA-Z][、.．)])[ \t]+',
    ...
    def _make_unit(self, context_lines: List[str], item: str, strategy: str,
                   marker: str, title: str) -> Dict[str, Any]:
        return {
    ...
    def _split_content_to_units(self, chunk: Dict, content: str,
                                context_lines: List[str],
                                level_index: int = 0) -> List[Dict[str, Any]]:
        """按可用层级切出结构单元；超长单元继续尝试下一层级。"""
        levels = self._marker_levels()
        chosen = None

        for idx in range(level_index, len(levels)):
            preamble, parts = self._split_by_line_markers(content, levels[idx]['pattern'])
            meaningful = [
                (marker, body) for marker, body in parts
        ...
        if preamble:
            if len(preamble) <= MIN_CHUNK_SIZE:
                ...
                units.append(self._make_unit(
                    context_lines, preamble, f"{level['name']}_preamble", '',
        ...
        for marker, body in parts:
            ...
            if len(full_text) > limit and idx + 1 < len(levels) and not self._has_table(body):
                child_context = list(base_context)
                child_context.append(marker.strip())
                child_units = self._split_content_to_units(chunk, body, child_context, idx + 1)
                if len(child_units) > 1 or child_units[0]['strategy'] != 'unsplit':
                    units.extend(child_units)
                    continue

            units.append(self._make_unit(
                base_context, item, level['name'], marker,
    ...
    def split_structured_chunk(self, chunk: Dict) -> List[Dict]:
        """
        结构优先，但不是每个编号都单独成块：
        1. 识别一、二、三 / 1、2、3 / （1）（2）等结构单元。
        2. 相邻同父级结构单元打包到约 1000 字。
        3. 结果超过上限时继续按更细层级或段落句子兜底。
        """
    ...
    def _merge_small_sub_chunks(self, chunks: List[Dict]) -> List[Dict]:
        ...
        for chunk in chunks:
            ...
            prev = merged[-1]
            same_parent = (
                prev.get('part') == chunk.get('part') and
                prev.get('chapter') == chunk.get('chapter') and
                prev.get('section') == chunk.get('section') and
                prev.get('article') == chunk.get('article')
            )
    ...
    def _pack_units_to_chunks(self, chunk: Dict, units: List[Dict[str, Any]]) -> List[Dict]:
        """把相邻同父级结构单元打包到接近 TARGET_CHUNK_SIZE。"""
        ...
        for unit in units:
            ...
            should_pack = (
                same_context and
                len(candidate_text) <= candidate_limit and
                (len(candidate_text) <= TARGET_CHUNK_SIZE or current_len < MIN_CHUNK_SIZE)
            )
    ...
    def _post_split_cleanup(self, chunks: List[Dict]) -> List[Dict]:
        """小块合并 + 对所有结果做最终超长检查。"""
    ...
    def _fallback_split_by_paragraph_or_sentence(self, chunk: Dict) -> List[Dict]:
        """最后兜底：优先按段落/行聚合，实在过长才按句子。"""
        text = chunk['content']
        ...
        result = []
        for part in parts:
            result.append(self._make_sub_chunk(
                chunk, part, chunk.get('sub_marker', ''),
                'fallback_paragraph_sentence', self._extract_sub_title(part)
            ))
    ...
    def merge_small_chunks(self, chunks: List[Dict]) -> List[Dict]:
        ...
        for chunk in chunks:
            if chunk['char_count'] < TINY_CHUNK_SIZE:
...
def generate_visualization(all_results: Dict[str, List[Dict]], output_dir: Path):
    ...
    html_parts = [
        ...
        "<html><head>",
        "<meta charset='utf-8'>",
        "<title>待审文档 Chunk 分析报告 v8</title>",
        "<style>",
        ...
        "</style>",
        "</head><body>",
        "<h1>📋 待审文档 - Chunk 分析报告 v8</h1>",
        f"<p style='color: #7f8c8d;'>生成时间: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')} | 目标: {TARGET_CHUNK_SIZE} | 最小合并: {MIN_CHUNK_SIZE} | 普通上限: {MAX_CHUNK_SIZE} | 表格上限: {TABLE_MAX_CHUNK_SIZE}</p>"
    ]
    ...
    html_path = output_dir / f"pending_doc_chunks_v8_{timestamp}.html"
    with open(html_path, 'w', encoding='utf-8') as f:
        f.write('\n'.join(html_parts))
    print(f"\n可视化报告已保存: {html_path}")

    # 保存JSON
    json_path = output_dir / f"pending_doc_chunks_v8_{timestamp}.json"
...
def main():
    ...
    for json_file in json_files:
        try:
            chunker = PendingDocChunkerV8(str(json_file))
